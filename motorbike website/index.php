...
if (isset($_POST['login'])) {
    ...
    for ($i = 0; $i < count($xml_data->NhanVien); $i++) {
        if (($username == $xml_data->NhanVien[$i]->TaiKhoanNV) and ($password == $xml_data->NhanVien[$i]->MatKhauNV)) {
            $_SESSION["user"] = $xml_data->NhanVien[$i]->TaiKhoanNV;
            $_SESSION['login'] = true;
            header("Location: quan_ly/admin.php");
        }
    }
...
<body>
    ...
    if ($SESSION['username'] == true) {
        ...
        echo '<div class="login-form-container" id="login-form">
